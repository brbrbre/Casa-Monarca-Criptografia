...
\newcommand{\fechaentrega}{14 de junio de 2026}
\newcommand{\cmark}{\ding{51}}   % marca de aprobado
\newcommand{\safeincludegraphics}[2][]{%
    \IfFileExists{#2}{\includegraphics[#1]{#2}}{%
        \fbox{\parbox{0.85\textwidth}{\centering Imagen no encontrada: \texttt{#2}}}%
    }%
}
...
\begin{table}[h!]
\centering
\caption{Tamaño de clave equivalente para 128 bits de seguridad (elaboración propia basada en NIST, 2023).}
\label{tab:keysizes}
\begin{tabular}{lcc}
\toprule
...
La figura 1 muestra el diagrama de arquitectura del sistema desarrollado para Casa Monarca. Las capas consisten en los usuarios, el frontend, los 2 modulos de la solucion, las bibliotecas y las herramientas utilizadas en las bases de datos.
...
\begin{itemize}
    ...
    \begin{center}
        ...
        \captionof{figure}{Pantalla de login con campos de usuario, contraseña y carga de \texttt{.cert}/\texttt{.key} (Niveles 1 y 2).}
        \label{fig:login}
    \end{center}
    Esta imagen aparece cuando un administrador o coordinador intenta iniciar sesión.
    \item Dashboard de colaboradores
    \begin{center}
        \includegraphics[width=0.85\textwidth]{dashboard.png}
        \captionof{figure}{Dashboard de colaboradores con tarjetas de estadísticas.}
        \label{fig:dashboard}
    \end{center}
    Listado de colaboradores de Casa Monarca.

    \item Onboarding
    \begin{center}
        \includegraphics[width=0.85\textwidth]{onboarding.png}
        \captionof{figure}{Formulario de onboarding de nuevos colaboradores.}
        \label{fig:onboarding}
    \end{center}
    Esta pantalla aparece cuando el colaborador quiere crear su usuario. El coordinador lo tiene que aprobar para que el colaborador se vuelva parte del sistema.

    \item Detalle de colaborador
    \begin{center}
        \includegraphics[width=0.85\textwidth]{detalle.png}
        \captionof{figure}{Vista de detalle de un colaborador con historial de acciones.}
        \label{fig:detalle}
    \end{center}
    Información detallada del colaborador, muestra la información personal, los datos y estado de su onboarding.

    \item Auditoría IAM
    \begin{center}
        \includegraphics[width=0.85\textwidth]{auditoria.png}
        \captionof{figure}{Módulo de auditoría IAM con filtros por fecha, acción y actor.}
        \label{fig:auditoria}
    \end{center}
    En la auditoría se puede rastrear la actividad de los colaboradores, solamente con el nivel necesario.

    \item Gestión de certificados
    \begin{center}
        \includegraphics[width=0.85\textwidth]{gestion.png}
        \captionof{figure}{Módulo de gestión de certificados con resumen por estado.}
        \label{fig:gestion}
    \end{center}
    En la gestión de certificados se puede verificar el estatus de los certificados y realizar distintas acciones de ser necesario.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{center}
        ...
        \captionof{figure}{Formulario de nuevo registro de migrante.}
        \label{fig:nuevo}
    \end{center}
    Si se busca registrar a un migrante, debe llevar a esta página. Se llena el formulario para agregarlo.
    \item Revisión y confirmación con firma
    \begin{center}
        \includegraphics[width=0.85\textwidth]{firma.png}
        \captionof{figure}{Pantalla de revisión y confirmación con solicitud de contraseña para firmar.}
        \label{fig:firma}
    \end{center}
    En caso de querer firmar un registro, se llega a esta pantalla, solo aplica para el nivel necesario.
    \item Listado de registros
    \begin{center}
        \includegraphics[width=0.85\textwidth]{listado.png}
        \captionof{figure}{Listado de registros de migrantes con buscador por identificador interno.}
        \label{fig:listado}
    \end{center}
    En la figura 10 aparece la forma en la que se enlistan a los migrantes en e modulo de registros.
    \item Detalle con firma ECDSA
    \begin{center}
        \includegraphics[width=0.85\textwidth]{ECDSA.png}
        \captionof{figure}{Detalle de registro con bloque de firma ECDSA verificada.}
        \label{fig:ECDSA}
    \end{center}
    En la parte superior de la figura 11 se observa donde el registro se firmó correctamente.

    \item Expediente completo
    \begin{center}
        \includegraphics[width=0.85\textwidth]{Expediente.png}
        \captionof{figure}{Expediente completo con línea de tiempo de eventos del registro.}
        \label{fig:Expediente}
    \end{center}
    En la figura 12 se puede visualizar el expediente de un migrante registrado previamente.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{center}
        ...
        \captionof{figure}{Listado de solicitudes de cambio con estados del flujo de aprobación.}
        \label{fig:listsolicitudes}
    \end{center}
    En esta pantalla aparece el listado de todas las solicitudes ARCO (en caso de que existan).

    \item Decisión con firma obligatoria
    \begin{center}
        \includegraphics[width=0.85\textwidth]{obligatoria.png}
        \captionof{figure}{Pantalla de decisión sobre solicitud con firma digital obligatoria.}
        \label{fig:obligatoria}
    \end{center}
    Para decidir sobre una solicitud arco, el administrador debe firmar, esto se observa en la figura 14.
    \item Formulario ARCO
    \begin{center}
        \includegraphics[width=0.85\textwidth]{ARCO.png}
        \captionof{figure}{Formulario de nueva solicitud de derechos ARCO.}
        \label{fig:ARCO}
    \end{center}
    En caso de que un migrante quiera ejercer un derecho ARCO, tendrá que llenar el formulario de la figura 15.

    \item Detalle de solicitud ARCO
    \begin{center}
        \includegraphics[width=0.85\textwidth]{DETARCO.png}
        \captionof{figure}{Detalle de caso ARCO con flujo de aprobación vinculado y plazo legal calculado.}
        \label{fig:DETARCO}
    \end{center}
    En caso de querer finalizar el ticket de la solicitud, aparece la pantalla de la figura 16. El administrador debe agregar sus credenciales para proceder.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{center}
        ...
        \captionof{figure}{Pantalla de cambio de contraseña.}
        \label{fig:cambio}
    \end{center}
    La figura 17 aparece si el usuario quiere cambiar su contraseña.
    \item Configuración de pregunta de seguridad
    \begin{center}
        \includegraphics[width=0.85\textwidth]{pregunta.png}
        \captionof{figure}{Configuración de pregunta de seguridad para recuperación de contraseña.}
        \label{fig:pregunta}
    \end{center}
    La figura 18 aparece cuando el usuario quiera configurar su pregunta de seguridad por si requiere recuperar su contraseña-
\end{itemize}
...
\textit{Nota:} No se incluyen capturas de todos los casos para mantener la proporción texto/imagen del documento. Las evidencias completas están disponibles en la carpeta \texttt{Casos de Prueba/} del repositorio \citeref{repo2026}{Álvarez Salas et al., 2026} y en los archivos adjuntos de casos de prueba.
...
A lo largo de las dos etapas del reto se diseñó e implementó un sistema funcional que apoya en la resolución de las necesidades de Casa Monarca. La Etapa 1 consistió en un gestor de identidades con trazabilidad completa mediante un espacio de auditoría.La Etapa 2 añadió un módulo de registro de migrantes que cifra los datos sensibles con AES-256, firma cada expediente con ECDSA sobre SHA-256, gestiona los cambios mediante aprobación por niveles y permite ejercer los cuatro derechos ARCO en los plazos legales. La cadena criptográfica de auditoría garantiza que el historial de acciones sea verificable e inalterable. Esto fortalece la confianza del sistema porque cada evento relevante queda asociado con un usuario, una fecha, una acción y una evidencia técnica que puede revisarse posteriormente. Con esta solución, Casa Monarca dispondrá de herramientas para responder solicitudes ARCO dentro del plazo legal y para detectar brechas de seguridad de forma temprana. Además, el sistema contribuye a ordenar procesos que antes podían depender de controles manuales o de registros dispersos.

En términos de usabilidad, las pantallas guían al usuario paso a paso, los mensajes de error son descriptivos y las acciones irreversibles requieren confirmación explícita con credenciales digitales. Esto reduce la probabilidad de errores humanos y permite que personal no técnico pueda operar el sistema con mayor seguridad. Desde una perspectiva criptográfica, la elección de ECDSA sobre SECP256K1 combinada con SHA-256 como función de pre-hash demostró ser adecuada para el contexto de la organización. Esta combinación ofrece 128 bits de seguridad con claves suficientemente compactas para que el personal las maneje como archivos .cert y .key sin ningún tipo de problema. El cifrado AES-256 de los campos sensibles garantiza que, incluso ante un acceso no autorizado directo a la base de datos MySQL, los datos personales de los migrantes permanezcan ilegibles. Esta protección es especialmente importante por la naturaleza vulnerable de la población atendida por Casa Monarca.

La cadena de auditoría criptográfica basada en SHA-256 añade una capa adicional de confianza. Cualquier intento de alterar el historial de acciones resulta detectable de forma matemáticamente verificable, lo que eleva la evidencia disponible para Casa Monarca ante cualquier problema legal o de cumplimiento normativo.Finalmente, el proyecto evidenció que es posible implementar un sistema de seguridad robusto dentro de las restricciones reales de una organización civil con recursos limitados. La solución no requiere costos de licenciamiento ni dependencias de infraestructura incompatible con el entorno disponible.

La decisión de construir sobre Django y MySQL demuestra que la seguridad de datos personales sensibles no es exclusiva de organizaciones con grandes presupuestos tecnológicos. Este principio de diseño resulta replicable por otras organizaciones de la sociedad civil que enfrenten retos similares de protección de datos en contextos de vulnerabilidad.

Como trabajo futuro, el primer paso consiste en desplegar el sistema en el servidor de Casa Monarca y acompañar este proceso con capacitación del personal. Esta capacitación debe cubrir el uso diario de la plataforma, la gestión de certificados, la atención de solicitudes ARCO y la respuesta ante incidentes.

También se propone el desarrollo de un mecanismo de respaldo automático de la base de datos. Este respaldo permitiría recuperar información ante fallas operativas, errores humanos o incidentes técnicos sin comprometer la continuidad del servicio.

De igual forma, se recomienda implementar un sistema de notificaciones por correo ante cualquier cambio irreversible. Estas alertas permitirían al administrador detectar movimientos críticos en tiempo oportuno, incluso cuando el cambio provenga de una cuenta aparentemente legítima.

Cuando los recursos de Casa Monarca lo permitan, se recomienda migrar a una autoridad certificadora de mayor madurez como step-ca. Esta alternativa ofrece gestión automatizada de renovaciones y soporte ante compromisos de seguridad \citeref{smallstep2026}{Smallstep Labs, 2026}.
...
\begin{description}[leftmargin=0pt, labelwidth=0pt, labelsep=0pt, itemindent=0pt, itemsep=8pt]
    ...
    \item[\hypertarget{ref:repo2026}{}]
    Álvarez Salas, M., Alvarez Valdez, B., Ruiz López, E., Nagao Alvarez, A. S., \& Estrada Ceferino, K. A. (2026). \textit{Casa-Monarca-Criptografia} [Repositorio de GitHub]. \url{https://github.com/brbrbre/Casa-Monarca-Criptografia}
...
\begin{longtable}{c p{1.6cm} p{2.4cm} p{2.8cm} p{3.0cm} c}
...
CP-07 & IAM --- Login & Onboarding pendiente & \texttt{status= pending} & Redirige a \texttt{/onboarding/} con mensaje informativo & \cmark \\[4pt]
CP-08 & IAM --- Login & Aprobado, sin cert descargado & \texttt{status= approved}; certificado no descargado & Redirige a pantalla de descarga de credenciales & \cmark \\[4pt]
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.85\textwidth]{61Resultado.png}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.85\textwidth]{63Resultado.png}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.85\textwidth]{65Resultado.png}
...
\begin{figure}[H]
\centering
\safeincludegraphics[width=0.85\textwidth]{68Resultado.png}
