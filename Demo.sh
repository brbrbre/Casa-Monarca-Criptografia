...
# Casa Monarca — Criptografía
# =============================================================================
# Este script levanta una CA interna, emite un certificado, lo verifica
# y firma un archivo simple para demostrar el flujo básico de PKI.
# =============================================================================
...
if ! command -v step &> /dev/null; then
    echo -e "${RED} 'step' no está instalado. Corre: brew install step${NC}"
    exit 1
fi

if ! command -v step-ca &> /dev/null; then
    echo -e "${RED} 'step-ca' no está instalado. Corre: brew install step-ca${NC}"
    exit 1
fi
...
# Inicializar la CA (solo si no existe ya)
echo -e "${YELLOW}[1/5] Inicializando la CA interna de Casa Monarca...${NC}"

if [ -f "$HOME/.step/config/ca.json" ]; then
    echo " Ya existe una configuración de step-ca en ~/.step"
    echo "   Si quieres empezar desde cero: rm -rf ~/.step"
    echo "   Continuando con la configuración existente..."
else
    # Inicialización no-interactiva de la CA
    step ca init \
    ...
    echo -e "${GREEN} CA inicializada correctamente${NC}"
fi
echo ""

#Levantar la CA en segundo plano
...
if ! kill -0 $CA_PID 2>/dev/null; then
    echo -e "${RED} La CA no pudo iniciar. Revisa el puerto 8443.${NC}"
    exit 1
fi

echo -e "${GREEN} CA corriendo (PID: $CA_PID)${NC}"
echo ""

# Emitir un certificado para un servicio
...
echo ""
echo -e "${GREEN} Certificado emitido: servicio.crt${NC}"
echo ""
...
echo -e "${GREEN} Certificado válido — la CA lo reconoce${NC}"
echo ""

# Firmar y verificar un archivo con la llave del certificado
...
echo "✍️  Firma generada: firma.bin"

# Verificar la firma con la llave pública del certificado
echo ""
